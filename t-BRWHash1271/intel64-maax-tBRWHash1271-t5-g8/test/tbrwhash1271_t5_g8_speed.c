...
int main(int argc, char **argv) {
	...
	if (msglen == 0) n = 1;
   	...
   	printf("CPU-cycles per byte for computing tbrwhash1271(t=5,g=8) using maax instructions on a message of length %llu bits: %6.4lf\n\n", msglen,((get_median())/(double)(n)));
   	...
   	printf("CPU-cycles per byte for computing tbrwhash1271(t=5,g=8) using maax instructions on a message of length %llu bits including time for computing key-powers: %6.4lf\n\n", msglen,((get_median())/(double)(n)));	
