...
def getKeyRandom(k):
	...
	key = []
	f = open("tbrwhash1271_t2_verify_keyfile.txt", "w")
	for i in range(k): 
...
def genTestCases():
	...
	t = 2;
	f1 = open("tbrwhash1271_t2_verify_test_cases.txt", "w")
	f2 = open("tbrwhash1271_t2_verify_s_outputs.txt", "w")	
	start = 1
	stop = 2040
	step = 1
