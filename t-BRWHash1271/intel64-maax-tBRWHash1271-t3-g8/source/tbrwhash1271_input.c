...
void format_msg(uchar8 *msg, int l,char *filename) {
	...
	fpout = fopen("./test_files/tbrwhash1271_msgfile_temp.txt","w");
    	if (fpout == NULL) {
	...
	fp = fopen("./test_files/tbrwhash1271_msgfile_temp.txt","r");
	...
	block_index = BLOCK_SIZE-1;
    	...
    	fclose(fp);
    	remove("./test_files/tbrwhash1271_msgfile_temp.txt");    	
}
