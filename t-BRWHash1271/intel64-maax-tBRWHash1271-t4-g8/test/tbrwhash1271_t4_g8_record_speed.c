...
int main() {
	...
	FILE *fpin,*fpout;
    	...
    	// #define SAGE
    	#ifdef SAGE     	
    	c = system("cd ./test_files; sage tbrwhash1271_t4_speed_record_test_cases.sage; cd ..");
    	remove("./test_files/tbrwhash1271_t4_speed_record_test_cases.sage.py");    	
    	#endif    	
    	   	
	format_key(k,KEY_LENGTH,"./test_files/tbrwhash1271_t4_speed_record_test_cases_keyfile.txt");
	q = (uint64 *)k; kp[2] = q[0]; kp[3] = q[1]; 
    	
   	fpin = fopen("./test_files/tbrwhash1271_t4_speed_record_test_cases.txt", "r");
    	if (fpin == NULL) {
	...
	fpout = fopen("./test_files/tbrwhash1271_t4_g8_speed_records.txt","w");
