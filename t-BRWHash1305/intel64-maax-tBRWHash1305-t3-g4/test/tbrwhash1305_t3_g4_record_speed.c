...
int main() {
	...
	FILE *fpin,*fpout;
    	...
    	// #define SAGE
    	#ifdef SAGE     	
    	c = system("cd ./test_files; sage tbrwhash1305_t3_speed_record_test_cases.sage; cd ..");
    	remove("./test_files/tbrwhash1305_t3_speed_record_test_cases.sage.py");    	
    	#endif    	
    	   	
	format_key(k,KEY_LENGTH,"./test_files/tbrwhash1305_t3_speed_record_test_cases_keyfile.txt");
	q = (uint64 *)k; kp[3] = q[0]; kp[4] = q[1]; kp[5] = 0;
    	
   	fpin = fopen("./test_files/tbrwhash1305_t3_speed_record_test_cases.txt", "r");
    	if (fpin == NULL) {
	...
	fpout = fopen("./test_files/tbrwhash1305_t3_g4_speed_records.txt","w");
